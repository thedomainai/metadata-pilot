...
def _compile_meta_for_file(file_path: str, cfg_path: str) -> None:
    ...
    # Extract title from filename if not already set or is default
    current_title = meta.get("text_title", "")
    text_defaults_cfg = cfg.get("text_defaults", {}) or {}
    default_fallback = text_defaults_cfg.get("text_title_fallback", "Untitled")
    
    if not current_title or current_title == "Untitled" or current_title == default_fallback:
        filename_without_ext = os.path.splitext(os.path.basename(abs_path))[0]
        # Clean up filename: replace underscores and hyphens with spaces
        title = filename_without_ext.replace("_", " ").replace("-", " ")
        # Preserve existing capitalization but normalize spaces
        title = " ".join(title.split())
        meta["text_title"] = title if title else "Untitled"
